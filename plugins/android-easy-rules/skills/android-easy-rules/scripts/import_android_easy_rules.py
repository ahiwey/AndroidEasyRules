...
- 默认先对新任务做轻量提示词增强，把原始表述整理为目标、范围、约束、关键假设和可验证成功标准；只展示影响理解、决策或验收的摘要。
- 非琐碎且会产生代码、文件、配置或外部写入的任务默认采用两阶段交付：先完成最小查证、必要采访并给出方案，等待用户明确确认后再实施；简单事实、纯只读分析、明确 Quick 小改及用户要求“直接做”或“无需方案”时可跳过方案门。
- 开始任务前判断背景、痛点、需求和成功标准是否清楚；缺少会改变方案或验收结果的关键信息时，使用苏格拉底式提问集中询问 1–3 个问题。信息充分时不机械采访；能自行查明的事实先查证。
- 方案阶段给出增强后的任务理解、实施步骤、影响范围、验证方式和待决策项；支持 `优化提示词：`、`先采访我：`、`先给方案：`、`直接做：` 四个显式入口。
- 用户确认方案后，把已确认方案视为实施边界；除非代码已变化、验证失败或出现新证据，不重复完整排查或询问已确认事项。
- 完成已确认的多步骤方案时，最终回复按原方案编号逐项标注“已完成/延期”、对应修改位置和验证结果；实施中范围变化时只说明增量与影响，避免用户重新核对是否漏项。
...
- 用户明确要求创建或更新 Skill 时，默认生成到可写的 `AndroidSampleSkill/skills/<skill-name>/`；找不到仓库时先询问路径。完成验证后说明是否已提交和发布，未提交时提醒提交。
- 工作流优化 Skill 修改根规则或项目规则时，把不含项目事实的通用规则同步回 AndroidEasyRules；当前电脑没有 AndroidEasyRules 时只更新本机和项目，不自动安装。
- 用户反馈“还是不对”“继续微调”“没画好”或“仍会复现”时视为返工信号：停止猜参数，重看最新证据、当前差异和运行时资源，再确认父布局、状态转换和可观察验收信号。
- 异步、缓存、首次进入、绑定/解绑或重复请求问题先列最小“状态 × 事件 × 期望输出”矩阵，并区分请求级结果、单次尝试事件和对象所有权。
- 业务定位先按关键词查 `MEMORY.md`，结构定位用 CodeGraph，固定文本和资源名用 `rg`。
- 调用依赖设备、外部工具或可选路径的一组命令前，先做一次低成本前置条件探测；探测失败后停止依赖调用。搜索候选文件前先确认实际存在的目录或从 `rg --files` 结果筛选；Windows 环境不假设 Unix 命令可用，复杂 PowerShell 变量或正则避免多层 `-Command` 转义。
