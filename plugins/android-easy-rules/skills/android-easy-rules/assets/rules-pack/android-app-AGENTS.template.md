...
- `<填写 app 模块名>` 是主应用模块，承载 UI、业务、网络、数据、权限、资源和设备交互入口。
- 开始主应用任务前，先在根目录 `MEMORY.md` 检索业务关键词，避免从整个 `<填写 app 模块名>` 全量扫描。
- 主代码位于 `<填写 app 主包路径>`。
...
- `<填写 app 模块名>` 资源或 XML 改动后，优先运行 `<填写 app debug assemble 命令>`。
