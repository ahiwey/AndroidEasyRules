...
- 如果根规则、全局规则或用户本轮要求默认不进行编译校验，则本节的 assemble/编译建议不自动执行；只有用户明确要求，或本次改动实际需要时才运行，并先说明原因。
- 现有资源属性值或 XML 布局参数调整不默认运行 Gradle；新增/重命名资源或 XML 引用时，最多运行一次对应变体的最小资源处理任务。
- 只有用户要求 APK、改动 manifest/构建/打包链路，或更小任务无法覆盖集成边界时，才运行 `<填写 app debug assemble 命令>`。
