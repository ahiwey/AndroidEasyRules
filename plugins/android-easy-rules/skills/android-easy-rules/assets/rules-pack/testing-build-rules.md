...
- 文档、规则、纯注释、数值参数和现有资源属性值调整通常不运行 Gradle；使用内容检查、`git diff --check` 和必要的截图/人工视觉核对。上一轮视觉修改已失败且只有运行时渲染能判定时，先询问是否做真机/模拟器自动化验证；已有明确授权则不重复询问。同意后才探测设备并进行一次最窄安装/启动与同场景截图；跳过或未获答复时标注运行态/视觉未验证，不升级为无关 assemble。
