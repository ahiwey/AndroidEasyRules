...
- 共享生成目录构建、同一设备安装与交互按会话串行安排；启动前空闲不保证独占。用已有协调渠道/锁确认执行者、源码/产物及释放条件，不新建调度框架或未经授权联系其他任务。无稳定窗口时继续独立静态工作，保留该阶段未验证。
- 首个失败区分本次源码、其他进行中源码、生成目录竞争与未知；有原因变化证据才重试。不要用 clean、改增量开关、换 package 或重复安装绕过竞争。安装前核对源码/产物，外部安装或跳转后重核版本/场景，只作废受影响证据并续首个缺项；不扩大修他人的未完成代码。

- 已授权设备验证时，在耗时构建前先核实当前账号/游客、入口所需资料/能力和外设连接；其他身份的存量资料不代表当前入口可用。保存测试限定临时数据及恢复范围，续验重新取基线并校验备份格式，区分不存在与读取失败。按本次行/key 恢复并核对无关数据和重启入口，记录未写入/已恢复/未恢复，不重置无关数据或改生产 exported 来强开页面。弹框代表链路覆盖取消不落库、保存回读及键盘；截图与安装成功不替代交互或设备端结果。

- 同批语言/文案先补齐再做最小验证，不因局部提示或范围文案分次 assemble；数据字段/单位换算改变则按实际风险选聚焦测试或最窄编译。
- 已授权的截图框架接入先查项目已验证配置及 AGP/Kotlin/KSP、传递依赖、Jetifier/覆盖率和渲染/运行桩 API，仅修实际兼容问题并记录作用域。不将某项目的版本锁定、排除插桩或 API 降级作为通用默认，不为截图改变普通单测或发布构建。

- 组件渲染、纯状态/存储、真实宿主/导航证据分层；手工 inflate 弹窗或替代 Context 须注明与真实窗口/生命周期的差异。pending fixture 不证明真实请求，存储断言尽量独立于截图框架。
- 时间相关 fixture 固定 now/Clock、时区、日期分组和默认时间戳；请求生命周期改动按风险覆盖重复提交、取消、切对象后的迟到响应和失败重试，沿用现有测试设施，不自动新增依赖。
- 长任务使用完成通知或有界等待，结束后一次读聚焦报告；中断保留阶段与产物，不连续 tail 日志或重跑已有效构建。UI tree 失败先区分命令、解析与页面状态，原因明确后最多一次针对性重试；截图不证明未执行的交互。
- 同一宿主 UI tree 持续失败不逐弹框重复探测；已有键盘导航时可在授权内操作真实控件，核对焦点、稳定画面与保存结果，重绘/返回后重新确认焦点。键盘操作不证明触点、手势或软键盘布局；没有可靠路径的具体交互保留未验证，不把工具失败等同业务失败。
