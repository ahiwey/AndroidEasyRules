...
- 文档、规则、纯注释、数值参数和现有资源属性值调整通常不运行 Gradle；使用内容检查、`git diff --check` 和必要的截图/人工视觉核对。上一轮视觉修改已失败时先复核已有证据与源码，完成实现和最小静态检查后按下方 UI 验收选择处理；只有已选设备路线才安装/启动与获取同场景截图，Paparazzi 不走设备。未选择或跳过时标注视觉/运行未验证，不升级为无关 assemble。
...
- Paparazzi 只证明指定主题、密度、字体比例、语言及 fixture 下的离屏渲染，不证明真实导航、BLE 或设备交互。使用项目真实布局/View，不复制一份理想 UI 作为被测对象。
- 先确认现有框架和实际 task；没有框架先确认接入，不自动新增依赖或切换 Roborazzi/设备。不将“选择 Paparazzi”扩大成安装/操作手机。
- 仅记录新 baseline 不算通过：打开产物对照设计的布局关系、图标、文字色与图表形态；零值和非零状态分别覆盖。只跑目标测试，不启动全量快照矩阵。
