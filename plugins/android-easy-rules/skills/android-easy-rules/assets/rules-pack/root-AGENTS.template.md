...
- 在系统、平台和安全约束之下，用户当前明确指令优先于 Skill、历史记忆和默认偏好；本文件只在当前项目范围内生效。
...
- 修改代码时，优先保持项目现有风格与结构。
- 可逆、低影响且测试只会复述实现的改动不专门补测试；既定验证通过后，除非出现新改动、新失败或未决疑点，不重复或扩大验证。
- 收尾时删除本次创建且后续无用的临时文件，不清理任务开始前已经存在或来源不明的文件。
- 只有用户明确要求子代理、委派或并行 Agent 工作，且任务存在真正独立、可并行的工作流时才委派；明确每项委派的输入、输出和完成标准，最终结论由主代理复核。

## 模型与技能路由

- 蓝湖或本地导出图驱动的多页面快速实现，在 `lanhu-android-fast-workflow` 已可用时优先使用该 Skill；默认不调用 MCP，按需读取专项规则，不自动叠加通用设计评审或全量视觉验收。缺少 Skill 时沿用项目规则且不自动安装，用户明确要求精修时再扩大视觉检查。
...
- 可用专项技能要精准触发：commit/分支迁移用 `$commit-migration`，R8/keep 规则用 `$r8-analyzer`，整体重设计或移动端体验评审按需用 `ui-ux-pro-max`，自定义 View/图表按 `AGENTS/custom-view-chart-rules.md`；当前会话未暴露某工具或技能时，不声称已使用。
...
- 项目事实、生产状态、历史决策和外部约定以本项目 `AGENTS.md` 及其明确指向的脚本、探针、决策记录和合同文件为准；全局规则和 Skill 不应虚构或覆盖这些事实。
